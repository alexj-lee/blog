...
<section class="mb-16 w-full" id="newsletter">
	<div
		class="my-4 w-full border-y border-blue-200 bg-blue-50 p-6 dark:border-gray-600 dark:bg-vblue sm:rounded sm:border-x"
	>
		...
		{#if isNewsletterOpen}
			...
			<form class="relative my-4" on:submit={onSubmit}>
				<input
					...
					autocomplete="email"
					required={true}
					class="mt-1 block w-full rounded-md border-gray-300 bg-white px-4 py-2 pr-32 text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:bg-vblue dark:text-gray-100"
				/><button
